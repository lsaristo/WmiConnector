...
namespace AutoBack {
    ...
    /// </summary>
    public class Driver {
        private static XDocument targetXML = null; //readFileToXML(Constants.TARGET_FILE);
        ...
        /// <summary>
        /// Main program entry point. Populate host list and execute commands against
        /// every host. 
        /// </summary>
        /// <param name="args">Desired host class to target. Refer to targets.xml</param>
        public static void Main(string[] args) {
            ...
            Lib.log(Constants.LL_INFO, "###############################");
            Lib.log(Constants.LL_INFO, "AutoBack has started ");
        ...
        /// <returns>String representation of param's value as defined in config.xml</returns>
        ...
        /// <summary>
        /// Parse program configuration options and set appropriate config variables. 
        /// </summary>
        private static void parseConfigOptions() {
            DEBUG_ENABLED = getConfigOption(Constants.DEBUG).ToLower().Equals(Constants.TRUE);
            LOG_ENABLED = getConfigOption(Constants.LOGGING).ToLower().Equals(Constants.TRUE);
            NO_EXECUTE = getConfigOption(Constants.WHATIF).ToLower().Equals(Constants.TRUE);
            targetXML = readFileToXML(getConfigOption(Constants.TARGETFILEPATH) + "\\"+
        ...
        private static void parseTargetFileXLS() {
            OleDbConnection conn = new OleDbConnection(
                ...
                +";Data Source="+getConfigOption(Constants.TARGETFILEPATH)
                ...
                if (classesToTarget.Contains(dr[@"Class"].ToString().ToLower())) {
                    remoteHostList.Add(
                        new RemoteHost {
                            ...
                            ,ArgsSetter = getConfigOption(Constants.EXECUTABLE_PATH)
                                  + "\\" + getConfigOption(Constants.EXECUTABLE_NAME)
                            ,Enabled = NO_EXECUTE ? false : 
                                dr["BU Enable"].ToString().ToLower().Equals(Constants.YES)
                    ...
                    foreach (XElement param in classTypes.Elements(Constants.HOST)) {
                        remoteHostList.Add(
                            new RemoteHost {
                                ...
                                ,ArgsSetter = getConfigOption(Constants.EXECUTABLE_PATH)
                                   + "\\" + getConfigOption(Constants.EXECUTABLE_NAME)
                                ,Enabled = NO_EXECUTE ? false : param.Element(Constants.ENABLED).Value.Equals(Constants.TRUE)
                                ...
                                ,RdiFile = getConfigOption(Constants.RDIMASTERPATH) 
                                    + "\\custom\\" + param.Element(Constants.HOST_NAME).Value + ".rdi"
